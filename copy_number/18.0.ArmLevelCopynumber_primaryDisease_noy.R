...
# saveRDS(out, "Analysis_folders/CopyNumberVariation/segmentationFiles/data/CopyNumber_summarised-to-pq-arms_primaryDisease.rds")
out = readRDS("Analysis_folders/CopyNumberVariation/segmentationFiles/data/CopyNumber_summarised-to-pq-arms_primaryDisease.rds") %>%
...
plot_dat = out %>%
  ...
  left_join(clin2 %>% select(wes, nice_name_collapsed, primary_met), by = c("sample" = "wes")) %>%
  filter(primary_met == "Primary")
...
between_samples_clustering = lapply(unique(clin2$nice_name_collapsed), function(x){
  samps = plot_dat[plot_dat$sample %in% clin2$wes[clin2$nice_name_collapsed == x],"sample"] %>% unique() %>% unlist()
  if(length(samps) < 2){
    return(samps)
  }
...
between_hist_clustering = plot_dat %>%
  ungroup() %>%
  select(Chr_Arm, `Arm CNt`, nice_name_collapsed) %>%
  group_by(Chr_Arm, nice_name_collapsed) %>%
  summarise(`Arm CNt` = mean(`Arm CNt`, na.rm = T)) %>%
  spread("Chr_Arm", "Arm CNt") %>%
  column_to_rownames("nice_name_collapsed") %>%
  as.matrix() %>%
  dist(., method = 'euclidean') %>%
  hclust(., method = "complete")
between_histology_clustering = between_hist_clustering$labels[between_hist_clustering$order]
#save histology order
#saveRDS(between_histology_clustering, "Analysis_folders/CopyNumberVariation/segmentationFiles/data/histology_order.rds")
...
pdf("Analysis_folders/CopyNumberVariation/segmentationFiles/results/All_Sarcoma_CN-by-arm_primaryDisease_noy.pdf", width = 20, height = 30)
...
pdf("Analysis_folders/CopyNumberVariation/segmentationFiles/results/All_Sarcoma_WGA_primaryDisease_noy.pdf", width = 20, height = 30)
pl
dev.off()