...
clin2 = clinical %>% 
  ...
  mutate(new_collapsed = ifelse(n() < 5, "other", cdc_nice_name)) %>% #if there are less than 5 samples, group them to Other
  ungroup()
...
cntools_tables = pbmclapply(clin2$wes, function(samp){ #for the WES sample IDs in the clinical file
  ...
  tmp_file = fread(paste0("data/WES/somatic_CNV/", cnTools_files[cnt_id]), data.table = F) %>% #fread in sample (data.table) 
    filter(.[[6]] != 2, !grepl("rRNA", genename)) %>% #remove those genes that are normal with 2 copies and rRNA genes
...
#save file
saveRDS(dat, "Analysis_folders/CopyNumberVariation/cnTable_maftools.rds")
