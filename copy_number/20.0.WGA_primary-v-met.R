...
cn_data %>%
  pivot_wider(names_from = "cn", values_from = "Length", values_fill = 0) %>%
  mutate(gain_length = GAIN + AMP,
         loss_length = HOMDEL + HETLOSS,
         longer = ifelse(gain_length > loss_length, "gain", "loss")) %>%
  janitor::tabyl(longer)
