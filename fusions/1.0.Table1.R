...
rna = clinical %>%
  filter(!is.na(rna_file), is.na(sarcoma), !is.na(preservation_method)) %>%
  group_by(nice_name_reassigned) %>%
  mutate(nice_name_reassigned_collapsed = ifelse(n() < 5, "Other", nice_name_reassigned)) %>%
  group_by(nice_name) %>%
  mutate(nice_name_collapsed = ifelse(n() < 5, "Other", nice_name)) %>% ungroup()

wes = clinical %>%
  filter(is.na(sarcoma), !is.na(somatic_file)) %>% #remove 'non-sarcoma' samples
  mutate(Tumor_Sample_Barcode = gsub("\\..*", "", somatic_file)) %>% #remove everything after "." to get sample ID
  group_by(nice_name_reassigned) %>% #group by the sarcoma histology Andrew collapsed
  mutate(nice_name_reassigned_collapsed = ifelse(n() < 5, "Other", nice_name_reassigned)) %>% #with a nice name called "Other" for plotting
  ungroup()
...
clin3 %>% 
  select(orien_avatar_key, nice_name_reassigned) %>%
  distinct() %>%
  group_by(nice_name_reassigned) %>%
  summarise(n()) %>%
  View()
...
wes_other = wes %>%
  ...
  summarise(n = n())
wes_other %>%
  unite("cleaner", nice_name_reassigned:n, sep=" (n = ") %>%
  pull(cleaner) %>%
  paste0(., collapse = "), ")
sum(wes_other$n)
...
total_tumors2 %>% 
  janitor::tabyl(nice_name_reassigned) %>%
  filter(n<5) %>%
  unite("cleaner", nice_name_reassigned:n, sep=" (n = ") %>%
  pull(cleaner) %>%
  paste0(., collapse = "), ")
