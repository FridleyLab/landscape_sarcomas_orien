---
title: "fgsea using ranked DE genes amomg immune groups"
---
...
# GSEA ON DE AMONG IMMUNEGROUPS
```{r include=F}
# Read DE genes and rank statistics (p-value, logFC)
de_fp = '../results/differential_expression_immunegroups/diff_expr_genes_immunegroups.xlsx'
de_res = list()
...
for(i in names(de_res)){
  ...
  openxlsx::write.xlsx(fgsea_res[[i]], paste0('../results/gene_set_enrichment_immunegroups/gene_set_enrichment_deg_immunegroup_', i, '.xlsx'))
}
...
for(i in names(fgsea_res)){
  for(j in names(fgsea_res[[i]])){
    ...
    df_tmp = bind_rows(df_tmp, fgsea_res[[i]][[j]] %>% filter(padj < 0.05) %>%
                         ...
                         add_column(immunegroup=i, dbase=j) %>%
                         mutate(order_bubble=NES-0) %>%
                         left_join(., geneset_tmp, by='pathway') %>%
                         mutate(percent_set_coverage=(size/gene_set_size)*100))
  }
}
...
for(j in c('hmk', 'keg', 'rct')){
  bb_p[[j]] = ggplot(df_tmp %>% filter(dbase == j)) +
    ...
    facet_wrap(~immunegroup, nrow=1)
}

pdf('../results/gene_set_enrichment_immunegroups/hallmark_gene_set_enrichment_deg_immunegroups.pdf', width=12)
print(bb_p[['hmk']])
dev.off()

pdf('../results/gene_set_enrichment_immunegroups/kegg_gene_set_enrichment_deg_immunegroups.pdf', width=15, height=12)
print(bb_p[['keg']])
dev.off()

pdf('../results/gene_set_enrichment_immunegroups/reactome_gene_set_enrichment_deg_immunegroups.pdf', width=50, height=40)
print(bb_p[['rct']])
dev.off()
...
# GSEA ON DE AMONG COLLAPSED IMMUNEGROUPS
```{r include=F}
# Read DE genes and rank statistics (p-value, logFC)
...
for(i in names(de_res)){
  ...
  openxlsx::write.xlsx(fgsea_res[[i]], paste0('../results/gene_set_enrichment_immunegroups/gene_set_enrichment_deg_collapsed_immunegroup_', i, '.xlsx'))
}
```
...
pdf('../results/gene_set_enrichment_immunegroups/hallmark_gene_set_enrichment_deg_collapsed_immunegroups.pdf', width=9, height=6)
print(bb_p[['hmk']])
dev.off()

pdf('../results/gene_set_enrichment_immunegroups/kegg_gene_set_enrichment_deg_collapsed_immunegroups.pdf', width=15, height=12)
print(bb_p[['keg']])
dev.off()

pdf('../results/gene_set_enrichment_immunegroups/reactome_gene_set_enrichment_deg_collapsed_immunegroups.pdf', width=50, height=40)
print(bb_p[['rct']])
dev.off()

rm(df_tmp, bb_p) # Clean env
```
