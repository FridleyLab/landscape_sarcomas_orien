...
key_df = key_df %>% 
  filter(!(rna_seq_mod %in% c("sl362871", "sl482620"))) %>% # Not true sarcomas (Andrew's opinion)
  filter(rna_seq_mod != "sl424998") # No library prep info on meta data, and unable to apply Combat
```

```{r include=F}
# Merge key with institution of origin for samples
inst_fp = '../data/SiteAnnotationSarcomaAvatars_19PRJ024RIG-SAR.xlsx'
...
```{r include=F}
# Read tumor mutation burden data (prepared by Alex Soupir)
...
umap_df = umap_res[['layout']] %>%
  ...
  left_join(., key_df %>%
              select(sample=rna_seq_mod, sarcoma_collapsed, immunegroup, 
                     specimen_type_collapsed, site_recode, nice_name_reassigned), by='sample')

rm(umap_res) # Clean env

# Save UMAP data frame to share with Nathan Seligson
#write.csv(umap_df, '../results/umap_coordinates_and_variables_nathanseligson_aug282023.csv', quote=F, row.names=F)
```

```{r include=F}
# Create UMAP plots colored by histologies
up1 = ggplot(umap_df) +
  geom_point(aes(x=V1, y=V2, color=nice_name_reassigned), size=0.7, alpha=0.9) +
  ggtitle('Sarcoma histology (UMAP)') +
...
up2 = ggplot(umap_df %>%
               mutate(umapcolor=ifelse(key_df[['nice_name_reassigned']] == 'Leiomyosarcoma', 'Leiomyosarcoma', 'Other'))) +
  geom_point(aes(x=V1, y=V2, color=umapcolor, size=umapcolor)) +
  ggtitle('Leiomyosarcoma') +
  xlab(paste0('UMAP1')) + ylab(paste0('UMAP2')) +
  labs(color=NULL) +
...
up3 = ggplot(umap_df %>%
               mutate(umapcolor=ifelse(key_df[['nice_name_reassigned']] == 'Gastrointestinal Stromal Tumor', 'Gastrointestinal Stromal Tumor', 'Other'))) +
  geom_point(aes(x=V1, y=V2, color=umapcolor, size=umapcolor)) +
  ggtitle('Gastrointestinal Stromal Tumor') +
  xlab(paste0('UMAP1')) + ylab(paste0('UMAP2')) +
  labs(color=NULL) +
...
up4 = ggplot(umap_df %>%
               mutate(umapcolor=ifelse(key_df[['nice_name_reassigned']] == "Myxoid Liposarcoma" , "Myxoid Liposarcoma", 'Other'))) +
  geom_point(aes(x=V1, y=V2, color=umapcolor, size=umapcolor)) +
  ggtitle("Myxoid Liposarcoma") +
  xlab(paste0('UMAP1')) + ylab(paste0('UMAP2')) +
  labs(color=NULL) +
...
up5 = ggplot(umap_df %>%
               mutate(umapcolor=ifelse(key_df[['nice_name_reassigned']] %in% c("Dedifferentiated Liposarcoma" , 
                                                                               'Liposarcoma', 
                                                                               'Liposarcoma (Well Differentiated)'), "Liposarcoma", 'Other'))) +
  geom_point(aes(x=V1, y=V2, color=umapcolor, size=umapcolor)) +
  ggtitle("Liposarcoma/Dedifferentiated Liposarcoma") +
  xlab(paste0('UMAP1')) + ylab(paste0('UMAP2')) +
  labs(color=NULL) +
...
annots_df_ordered = key_df %>%
  select(c('rna_seq_mod', 'immunegroup', 'nice_name_reassigned', 'primary_met', 'log_total_perMB')) %>%
  mutate(nice_name_reassigned_lipogrp=ifelse(key_df[['nice_name_reassigned']] %in% c("Dedifferentiated Liposarcoma" , 
                                                                                     'Liposarcoma', 
                                                                                     'Liposarcoma (Well Differentiated)'), "Liposarcoma", nice_name_reassigned)) %>%
  arrange(nice_name_reassigned_lipogrp, nice_name_reassigned) %>%
...
hm_ann = HeatmapAnnotation(df=annots_df_ordered[match(colnames(counts_hm_mtx), rownames(annots_df_ordered)), ] %>%
                             select(c('nice_name_reassigned', 'primary_met')),
...
mcp_res = mcp_res %>% column_to_rownames('cell_type')
...
hm_ann = HeatmapAnnotation(df=annots_df_ordered %>%
                           ...
                           col=list(immunegroup=col_pal_dtc,
                                    nice_name_reassigned=col_pal_hist,
                                    primary_met=col_pal_met))
...
#rm(mcpctr_hm, gexpr_hm, hm_list, hm_ann_bar) # Clean env
```
...
all_samples_tmp = key_df %>%
  ...
  left_join(., key_df %>%
              group_by(nice_name_reassigned, immunegroup) %>%
              summarize(histology_immune_samples=n(), .groups='drop') %>%
              ungroup(), by='nice_name_reassigned', multiple='all') %>%
  mutate(hist_immune_prop=histology_immune_samples/histology_samples) %>%
  mutate(labels_n=paste0(nice_name_reassigned, ' (', histology_samples, ')'))

# Calculate proportion of immunegroup E to sort bars
for(i in unique(all_samples_tmp[['nice_name_reassigned']])){
  prop_tmp = as.vector(unlist(all_samples_tmp[['hist_immune_prop']][all_samples_tmp[['nice_name_reassigned']] == i & all_samples_tmp[['immunegroup']] == 'E']))
  if(length(prop_tmp) > 0){
    all_samples_tmp[['prop_group_e']][all_samples_tmp[['nice_name_reassigned']] == i] = prop_tmp
  } else{
    all_samples_tmp[['prop_group_e']][all_samples_tmp[['nice_name_reassigned']] == i] = 0
  }
  rm(prop_tmp) # Clean env
}

# Ger total number of samples
total_samples = all_samples_tmp %>% select(c('nice_name_reassigned', 'histology_samples')) %>% 
...
all_samples_tmp = bind_rows(all_samples_tmp,
                            tibble(nice_name_reassigned='All histologies',
...
label_order = all_samples_tmp %>% select(c('nice_name_reassigned', 'prop_group_e')) %>% 
...
for(i in unique(all_samples_tmp[['nice_name_reassigned']])){
  ...
  if(i == "All histologies"){
    ...
    df_tmp = key_df %>% filter(nice_name_reassigned == i) %>% 
      select(c('immunegroup', 'primary_met')) %>% table() %>% as.data.frame.matrix()
  }
  ...
  result_ptest = bind_rows(result_ptest, tibble(nice_name_reassigned=i, p_val=ftest[['p.value']]))
  if(!is.na(ftest[['p.value']])){
    if(ftest[['p.value']] < 0.05){
      new_label = paste0(all_samples_tmp[['labels_n']][all_samples_tmp[['nice_name_reassigned']] == i], ' *')
    } else if(ftest[['p.value']] >= 0.05){
      new_label = paste0(all_samples_tmp[['labels_n']][all_samples_tmp[['nice_name_reassigned']] == i], '')
    }
  }else{
    new_label = paste0(all_samples_tmp[['labels_n']][all_samples_tmp[['nice_name_reassigned']] == i], ' x')
  }
  all_samples_tmp[['labels_n']][all_samples_tmp[['nice_name_reassigned']] == i] = new_label
...
bp_all = ggplot(all_samples_tmp %>%
                  left_join(., label_order, by='nice_name_reassigned'), aes(y=reorder(labels_n, desc(label_order)), x=hist_immune_prop)) +
...
uncorr_umap_df = uncorr_umap_res[['layout']] %>%
  ...
  left_join(., key_df %>%
              select(sample=rna_seq_mod, immunegroup, specimen_type_collapsed, 
                     site_recode, nice_name_reassigned), by='sample')
```
...
ucup1 = ggplot(uncorr_umap_df) +
  ...
  ggtitle('UMAP (uncorrected counts)\nPreservation method') +
  xlab(paste0('UMAP1')) + ylab(paste0('UMAP2')) +
  labs(color=NULL) +
...
up5 = ggplot(umap_expr_df) +
  ...
  labs(color='Norm.\nexpr') +
  #khroma::scale_color_sunset(midpoint=median(umap_expr_df[['norm_expr']])) +
